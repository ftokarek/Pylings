...
def validate_positive(number: int) -> int:
    """
    Return number if positive, otherwise raise NegativeNumberError.
    """
    if number < 0:
...
def check_age(age: int) -> str:
    """
    Validate age. Raise InvalidAgeError if invalid.
    """
    if age < 0 or age > 120:
...
def withdraw_money(balance: int, amount: int) -> str:
    """
    Withdraw money if balance is sufficient, otherwise raise InsufficientFundsError.
    """
    if amount > balance:
...
try:
    ...
    result_one = str(e)
    assert result_one == "Negative numbers are not allowed.", (
        f"Expected: Negative numbers are not allowed., but got {result_one}"
    )
...
try:
    ...
    result_three = str(e)
    assert result_three == "Invalid age.", (
        f"Expected: Invalid age., but got {result_three}"
    )
...
try:
    ...
    result_five = str(e)
    assert result_five == "Insufficient funds.", (
        f"Expected: Insufficient funds., but got {result_five}"
    )

result_six = withdraw_money(200, 100)
assert result_six == "Transaction successful.", (
    "Sufficient funds should allow withdrawal."
)
